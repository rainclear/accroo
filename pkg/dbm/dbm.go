...
func OpenDb() error {
	...
	rows, err := db.Query("Select AccountType from AccountTypes Order by id;")
	if err != nil {
		return err
	}
	...
	for rows.Next() {
		...
		if err = rows.Scan(&account_type); err != nil {
			return err
		}
		app.AccountTypes = append(app.AccountTypes, account_type)
	}
